...
their burst times in decreasing order so that the last created child process
is finished and exited first, then the second last one created is exited next,
and so on.
...
int main(int argc, char *argv[])
{
    ...
    // store order of of processes as they exit
    for (int i = 0; i < N; i++)
    {
        retorder[i] = wait();
    }
